...
REGISTER MAPPING
  ...
  r15   Callee Save -> 
*)
let rec create_arg_space (cont: ctxt) (index: int) (params: (ty * Ll.operand) list ) : X86.ins list =
  match params with
  | [] -> []
  | (h_ty, h_op)::tl when index <= 6 -> (*We still have space left in the registers*)
    begin
      (compile_operand cont (Reg R11) h_op)::(calling_arg_create (Reg R11) index)::(create_arg_space cont (index+1) tl)
    end
  | ls -> (*We have no more space left in the registers*)
    begin
      fold_left_map (fun (ind: int) (typ, op) -> 
        (ind+1, [compile_operand cont (Reg R11) op; Pushq, [Reg R11] ]))
      index (List.rev ls)   |>
      snd |>
      List.flatten
    end
...
let compile_call (cont: ctxt) (label: uid) (return_type: ty) (func: Ll.operand)
  ...
  let reg_save_list = [Reg Rbx; Reg Rcx; Reg Rdx; Reg Rsi; Reg Rdi; Reg R08; Reg R09;
  Reg R10; Reg R11; Reg R12; Reg R13; Reg R14; Reg R15 ] in
  (*Push saved register onto the stack*)
  (*let p1 = (Pushq, [(Reg Rbp)]) in
  let p2 = (Pushq, [(Reg Rsp)]) in *)
  (reg_save_list |>
  List.map (fun op -> (Pushq, [op]) ) ) @

  (*Push all the arguments onto the stack in order*)
  (*pseudo-reverse the argument list. The arguments in the registers are fine, which is how most of the cases pass.
  However once we get to the stack all the arguments are pushed on in reverse order. *)
  create_arg_space cont 1 params @
  ...
  (* Pop the arguments back off of the stack*)
  let sub_amount = List.length params-6 |> max 0 in
  list_make (Popq, [Reg R09]) sub_amount @
  (*Pop saved registers off of the stack back into their registers*)
  (List.rev reg_save_list |>
  List.map (fun op -> (Popq, [op]) ) )
...
let compile_gep (cont: ctxt) (op : Ll.ty * Ll.operand) (path: Ll.operand list) : ins list =
  ...
  (Imulq, [Imm (Lit (Int64.of_int @@ size_ty cont.tdecls sub_type)); Reg R11])::
...
let compile_insn (cont: ctxt) ( (label: uid), (i: Ll.insn) ) : X86.ins list =
  ...
  | Icmp ((c:Ll.cnd), (t:ty), (op1:Ll.operand), (op2:Ll.operand)) -> begin (*Update to
    1 if the comparison is true; update to 0 if it is false.*)
      ...
      (Cmpq, [Reg R12; Reg R11]):: (*Compare and set*)
