...
(*Function that only prints out a string if verbosity is set*)
let verb_go action = 
  if !Platform.verbose then action

(*Undirected graph type*)

type ukey = uid
type uvalue = UidS.t
type uGraph = uvalue UidM.t

(*Adds edges between every single node in the set, to every other node in the set*)
let add_set (g:uGraph) (set: uvalue) : uGraph =
  (*Adds an edge between the key and every node in the set to an undirected graph. All nodes
  contain a reflexive edge to themselves.*)
  let add_edges (g:uGraph) (first: ukey) (new_set: uvalue) : uGraph =
    match UidM.find_opt first g with
    | Some _ -> UidM.update (UidS.union new_set) first g
    | None -> UidM.add first new_set g
  in
  UidS.fold (fun node grph -> add_edges grph node set) set g

(*Removes an edge only in the direction of first to second. Modifies the set contained by first, to remove
the second.*)
let remove_directed_edge (g:uGraph) (first: ukey) (second: ukey) : uGraph =
  UidM.update (UidS.remove second) first g

(*Pick a node at random out of the graph. First tries to pick a node that has degree < K.
Then tries to pick the node with the maximum amount of edges*)
let rand_node (g:uGraph) (k:int) : (ukey*uvalue) option =
  match UidM.find_first_opt (fun uid ->
    (UidM.find uid g |> UidS.cardinal) < k ) g  with
  | Some (key, value) as result -> result
  | None -> begin (*There are no nodes with degree < k*)
    let choose_max_value (new_key: ukey) (new_value: uvalue) (acc: (ukey * uvalue) option) : (ukey * uvalue) option =
      match acc with
      | Some (old_key, old_value) -> begin
        if UidS.cardinal old_value < UidS.cardinal new_value then Some (new_key, new_value)
        else Some (old_key, old_value)
        end
      | None -> Some (new_key, new_value)
    in
    UidM.fold choose_max_value g None
  end

type color = int
type cmap = color UidM.t

let cmap_to_string (m: cmap) : string =
  UidM.fold (fun key value acc -> key ^ "(" ^ (Int.to_string value) ^ ") ") m "[" ^ "]" 

let seq_to_string seq : string = 
  Seq.fold_left (fun acc elem -> acc ^ " " ^ elem) "" seq

(*Creates a map of uid's to colors. Anything that is spilled is given a max_color+1 value.
So the graph is never realistically colored if it isn't k-colorable. All the function parameters should
already be described within the base color map.*)
let color_mapping (base: cmap) (max_color : color) : uGraph -> cmap =
  (*TODO handle the reflexive edge that exists in the neighboors*)
  (*A function that gives the best color to assign to a node, given the colors of all of its neighbors. Also
  appends the new node and new color into the existing color map. Finds the minimum color that isn't already used.
  If the node is already in the color map, we ignore it and accept the value that's already in there. *)
  let add_color (node: ukey) (neighbors: UidS.t) (col_map: cmap) : cmap =
    (*Find a suitable color based on neighboors colors. *)
    let neigh_colors = UidS.to_seq neighbors |> Seq.map (fun id -> begin 
      match UidM.find_opt id col_map with
      | Some thing -> thing
      | None -> verb_go @@ Printf.printf "Color of uid %s not found in %s\n" id (cmap_to_string col_map);
        failwith "Couldn't find color in color map" end) in
    let bad_col col : bool = Seq.fold_left (fun acc elem -> acc || (elem = col)) false neigh_colors in
    let rec new_color (test: color) = if bad_col test then succ test |> new_color else test in
    verb_go @@ Printf.printf "Calculating color of %s based off neighboors %s.\n" node (UidS.to_string neighbors);
    if UidM.mem node col_map then col_map else
      let id_col = (new_color 0) in
      verb_go @@ Printf.printf "Assigned the color %s(%d)\n" node id_col;
    UidM.add node id_col col_map
  in
  (*A recursive function that will generate the color map from scratch. Each subcall creates
  an entirely new graph, so it'll be slow as hell. If I have time, replace the call stack of graphs with
  a stack of edges that should be added back in.*)
  let rec create_mapping (g:uGraph) : cmap =
    if UidM.is_empty g then base else
    (*Get a node from the graph. This node will have an unknown degree. We calculate spilling with the
    find_color function as written above.*)
    match rand_node g max_color with
    | Some (node, neighbors) -> begin 
      UidS.fold (fun neigh iter_g -> (*For each neighboor*)
        UidM.update (UidS.remove node) neigh iter_g) (*Remove the edge to the current node*)
        neighbors g (*Act on the current working version of the outer graph*)
      |> UidM.remove node (*Remove the working node itself from the graph*)
      |> create_mapping (*Create a color mapping with the subgraph*)
      |> add_color node neighbors (*Put the current working node into that color map*)
      end
    | None -> begin
      failwith "(backend.ml 796) No node found in non-empty graph. Should not be possible.\n"
      end
  in
  create_mapping

  (*The better layout function that creates the better layout*)
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  verb_go @@ Printf.printf "Starting Better allocation of registers\n";
  (*Create the undirected graph to store all of the uids*)
  let (g : uGraph)= fold_fdecl
  (fun grph _ -> grph) (*Function arguments:  We can ignore the function arguments since I take the
  live_in from everything else, I must calculate the values according to the first instruction,
  which has only the arguments in it's live_in*)
  (fun grph lab -> add_set grph @@ live.live_in lab) (*Function labels*)
  (fun grph (id,ins) -> add_set grph @@ live.live_in id) (*Normal Instructions*)
  (fun grph (id,term) -> ((live.live_in id |> add_set grph ) |> add_set) @@ live.live_out id ) (*Terminators:
  Since these occur at the end of every block, we take both the live_in and live_out from them, since their live_out
  has a chance of never being seen again.*)
  UidM.empty f in
  ...
  *) 
  ...
  (*The total number of registers we are allowed to use*)
  let k = LocSet.cardinal pal in

  (*A list of Callee save registers that we have access to*)
  let reg_list = LocSet.elements pal in

  (*A translation from a number to a stack location, number is a positive integer*)
  let spill num = Alloc.LStk (- num) in

  (*Translate a color to a location*)
  let color_to_location (c : color)  : Alloc.loc =
    (*If it can be put in a register, we put it in a register*)
    if c < k then List.nth reg_list c else
    (*Else we spill, starting with 1 offset from the stack pointer*)
    c - k + 1 |> spill
  in

  (*TODO create the color mapping that already has the arguments colored.*)
  let arg_color_map = UidM.empty in

  (*Create the color mapping, and change it into a location mapping*)
  verb_go @@ Printf.printf "Creating the actual color mapping\n";
  let col_map = color_mapping arg_color_map k g in
  let loc_map = UidM.map color_to_location col_map in

  (*Calculate the maximum color, and therefore the max spillage*)
  let max_spill = 
    let max_col = UidM.fold (fun _ col acc -> max col acc) col_map 0 in
    if max_col < k then 0 else max_col - k + 1 
  in

  verb_go @@ Printf.printf "Register allocation optimization complete\n";
  { uid_loc = (fun id -> UidM.find id loc_map)
  ; spill_bytes = 8 * max_spill
  }
