...
let rev_t (l: 'a list) : 'a list =
  let rec rev_t_helper (forward : 'a list) (reverse : 'a list) : 'a list = begin
    match forward with
    | [] -> reverse
    | h::tl -> rev_t_helper tl (h::reverse)
    end
  in
  rev_t_helper l []
...
let rec optimize (e:exp) : exp =
  ...
  | Mult ( e1, e2 ) ->  begin
    ...
    | (Const( 1L ), e2 ) -> e2
    | (e1, Const( 1L )) -> e1
    | (Const( 0L ), e2 ) -> Const( 0L )
    | (e1, Const( 0L )) -> Const( 0L )
    | _ -> Mult( e1, e2 )
    end
  | Neg( e ) -> begin
    match optimize e with
    | Const( e2 ) -> Const( Int64.neg e2 )
    | Neg( e2 ) -> e2
    | _ -> Neg( e )
    end
